...
- `decks/templates/deck.css` — Single source of truth for all deck styles. 4px baseline grid, 12-column layout, light/dark theme via CSS variables (`--bg`, `--text`, `--accent`, `--surface`). Accent: `#FF2F00` (light), `#FF4500` (dark).
...
3. **Match media to aspect ratio** — Use the appropriate variant class (`.split-square` for 1:1, `.split-strip` for wide)
4. **Presentation covers are standalone** — They don't use `deck.js`; they have their own inline theme toggle
